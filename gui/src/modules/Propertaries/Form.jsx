...
export default function Form() {
  const { state, errors, handleChange, handleSubmit, handleBlur } =
    useValidatorForm(PropertaryStore.propertary, clientValidate, PropertaryStore.addproperty);

  const { id,name, email, lastnames, phone, address, dni } = state;

  const savePropertary = async (e) => {
    handleSubmit(e);
  };
      ...
      <Box display="flex" alignItems="center">
        <Card sx={{ p: 4 }}>
          <Typography variant="h6" sx={{ mb: 2 }}>
            {id ===null? 'Registrar Nuevo Propietario' : 'Editar Propietario'}
          ...
          <DialogActions sx={{ p: 3 }}>
            <Button
              onClick={() => {
                PropertaryStore.setHiddenForm(false);
                PropertaryStore.setEditing(false);
              }}
